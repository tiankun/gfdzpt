...
public class Sys_data_scopeAction extends BaseAction {
	...
	public String searchList(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(request.getParameter("pageType") != null && !request.getParameter("pageType").equals("") 
			...
			return "/sysAdmin/sys_data_scopeSelect.jsp";
		return "/sysAdmin/sys_data_scopeList.jsp";
	}
	public String searchById(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		buildDDL(request);
		return "/sysAdmin/sys_data_scopeDtl.jsp";
	}
	private Page search(HttpServletRequest request,int pageNo,int pageSize) throws Exception 
	{
		Map searchMap=getParameterMap(request);
		String sql="select id,roleid,(select rolename from sysrolesdic where id=roleid) RoleName,data_scope_type,dataid,"
				+"case data_scope_type when '0' then (select branchname from mrbranch where id=dataid) "
				+"when '1' then (select name from hr_base_info where id=dataid) end as DataName "
				+"from Sys_data_scope where 1=1 /~ and id={id} ~/"
		+ "/~ and roleid={roleid} ~/ "
		+ "/~ and data_scope_type={data_scope_type} ~/ ";
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		dataBaseControl.executeSql("DELETE FROM Sys_data_scope WHERE ID in (?)", new Object[]{s_id});
		
		return "/sysAdmin/Sys_data_scope!searchList.do";
	}
	private void buildDDL(HttpServletRequest request) throws Exception{
		
		request.setAttribute("data_scope_type",codeTableUtil.getCodeMap("data_scope_type")); 
	}
