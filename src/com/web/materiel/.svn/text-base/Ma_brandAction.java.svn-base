...
public class Ma_brandAction extends BaseAction {
	...
	public String searchList(HttpServletRequest request,HttpServletResponse response) throws Exception {
			...
			&& request.getParameter("pageType").equals("select")) 
		{
			...
			return "/materiel/brand/ma_brandSelect.jsp";
		}
		return "/materiel/brand/ma_brandList.jsp";
	}
	public String searchById(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		buildDDL(request);
		return "/materiel/brand/ma_brandDtl.jsp";
	}
	private Page search(HttpServletRequest request,int pageNo,int pageSize) throws Exception 
	{
		Map searchMap=getParameterMap(request);
		String sql="select * from Ma_brand where 1=1 and yxbz='1' /~ and id={id} ~/"
		+ "/~ and name like '%[name]%' ~/ "
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		dataBaseControl.executeSql("DELETE FROM Ma_brand WHERE ID in (?)", new Object[]{s_id});
		
		return "/materiel/Ma_brand!searchList.do";
	}
