...
public class Hr_rewardAction extends BaseAction {
	...
	public String searchList(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(request.getParameter("pageType") != null && !request.getParameter("pageType").equals("") 
			...
			return "/hr/info/hr_rewardSelect.jsp";
		return "/hr/info/hr_rewardList.jsp";
	}
	...
	{
		Map searchMap=getParameterMap(request);
		String sql="select * from Hr_reward where 1=1 /~ and id={id} ~/";
		XsqlFilterResult xsql = new XsqlBuilder().generateSql(sql,searchMap); 
		Page page=dataBaseControl.getPageResultSet(xsql.getXsql(), xsql.getAcceptedFilters().values().toArray(),pageNo,pageSize);
		return page;
	}
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String pidString=(dataBaseControl.getOneResultSet("select p_id from Hr_reward where id=?",  new Object[]{id})).get("p_id").toString();
		
		dataBaseControl.executeSql("DELETE FROM Hr_reward WHERE ID in (?)", new Object[]{id});
		
		return "/hr/info/Hr_reward!searchList.do?id="+pidString;
	}
	private void buildDDL(HttpServletRequest request) throws Exception{
		
		request.setAttribute("reward_type",codeTableUtil.getCodeMap("reward_type")); 
	}
