...
public class Hr_compactAction extends BaseAction {
	...
	public String searchList(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(request.getParameter("pageType") != null && !request.getParameter("pageType").equals("") 
			...
			return "/hr/info/hr_compactSelect.jsp";
		return "/hr/info/hr_compactList.jsp";
	}
	...
	{
		Map searchMap=getParameterMap(request);
		String sql="select * from Hr_compact where 1=1 /~ and id={id} ~/";
		XsqlFilterResult xsql = new XsqlBuilder().generateSql(sql,searchMap); 
		Page page=dataBaseControl.getPageResultSet(xsql.getXsql(), xsql.getAcceptedFilters().values().toArray(),pageNo,pageSize);
		return page;
	}
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String pidString=(dataBaseControl.getOneResultSet("select p_id from Hr_compact where id=?",  new Object[]{id})).get("p_id").toString();
		
		dataBaseControl.executeSql("DELETE FROM Hr_compact WHERE ID in (?)", new Object[]{id});
		
		return "/hr/info/Hr_compact!searchList.do?id="+pidString;
	}
	private void buildDDL(HttpServletRequest request) throws Exception{
		
		request.setAttribute("compact_type",codeTableUtil.getCodeMap("compact_type")); 
		request.setAttribute("fang_shi",codeTableUtil.getCodeMap("fang_shi")); 
	}
