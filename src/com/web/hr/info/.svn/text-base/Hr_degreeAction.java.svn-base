...
public class Hr_degreeAction extends BaseAction {
	...
	public String searchList(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if(request.getParameter("pageType") != null && !request.getParameter("pageType").equals("") 
			...
			return "/hr/info/hr_degreeSelect.jsp";
		return "/hr/info/hr_degreeList.jsp";
	}
	public String searchById(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		buildDDL(request);
		return "/hr/info/hr_degreeDtl.jsp";
	}
	private Page search(HttpServletRequest request,int pageNo,int pageSize) throws Exception 
	{
		Map searchMap=getParameterMap(request);
		String sql="select * from Hr_degree where 1=1 /~ and p_id={id} ~/ order by degree desc";
	...
	{
		Map searchMap=getParameterMap(request);
		String sql="select * from Hr_degree where 1=1 /~ and id={id} ~/";
		XsqlFilterResult xsql = new XsqlBuilder().generateSql(sql,searchMap); 
		Page page=dataBaseControl.getPageResultSet(xsql.getXsql(), xsql.getAcceptedFilters().values().toArray(),pageNo,pageSize);
		return page;
	}
	...
	public String delete(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String pidString=(dataBaseControl.getOneResultSet("select p_id from Hr_degree where id=?",  new Object[]{id})).get("p_id").toString();
		
		dataBaseControl.executeSql("DELETE FROM Hr_degree WHERE ID in (?)", new Object[]{id});
		
		return "/hr/info/Hr_degree!searchList.do?id="+pidString;
	}
	private void buildDDL(HttpServletRequest request) throws Exception{
		request.setAttribute("degree_type",codeTableUtil.getCodeMap("degree_type"));
		request.setAttribute("bi_ye",codeTableUtil.getCodeMap("bi_ye"));
		request.setAttribute("xian_shi",codeTableUtil.getCodeMap("bool"));
		request.setAttribute("degree",codeTableUtil.getCodeMap("degree"));
		request.setAttribute("xue_wei",codeTableUtil.getCodeMap("xue_wei"));
	}
