...
public class SqlTool {
	...
	public static String removeSelect(String sql) {
	    ...
	    if(sql.toUpperCase().indexOf("SELECT")==-1)   
	        totalSql="SELECT COUNT(*) "+sql.split("order by")[0].split("ORDER BY")[0];   
	    else if(sql.toUpperCase().split("SELECT").length==2){   
	        totalSql=sql.split("order by")[0].split("ORDER BY")[0];   
...
//			return sql.substring(beginPos);
	 ...
	 * @throws Exception 
	 */
	public static String getPageSqlByOrder(String sql,String type,int pageNumber,int pageSize,String sortname,String sortorder) throws Exception {
		if((type.indexOf("MYSQL")>=0)||(type.indexOf("SQLITE")>=0))
			sql+=" order by "+sortname+" "+sortorder+" LIMIT "+((pageNumber-1)*pageSize)+","+pageSize;
		else if(type.indexOf("ORACLE")>=0)
			sql="SELECT * FROM (SELECT a_.*,ROWNUM RN FROM ("+removeOrders(sql)+" order by "+sortname+" "+sortorder+")a_ WHERE ROWNUM <= "+(pageNumber*pageSize)+")b_ WHERE b_.RN>="+((pageNumber-1)*pageSize+1); 
		else if(type.indexOf("SQL SERVER")>=0){
			sql="SELECT TOP "+pageSize+removeOrders(sql).substring(7) +" WHERE ID NOT IN (SELECT TOP "+((pageNumber-1)*pageSize)+" ID "+removeSelect(sql)+")"+" order by "+sortname+" "+sortorder;
		}
		else if(type.indexOf("DB2")>=0)
			sql="SELECT * FROM (SELECT rownumber() AS RN, "+sql.substring(7)+" order by "+sortname+" "+sortorder+") WHERE RN BETWEEN "+((pageNumber-1)*pageSize+1)+" AND "+(pageNumber*pageSize); 
		else
			throw new Exception("========对不起，此数据库暂不被支持智能分页========");
		return sql;
	}	
	 ...
	 * @return
	 */
	public static String getTopSqlByOrder(String sql,String type,int top,String sortname,String sortorder) throws Exception {
		if((type.indexOf("MYSQL")>=0)||(type.indexOf("SQLITE")>=0))
			sql+=" order by "+sortname+" "+sortorder+" LIMIT "+top;
		else if(type.indexOf("ORACLE")>=0)
			sql="SELECT * FROM (SELECT a_.*, ROWNUM row_num FROM ("+removeOrders(sql)+" order by "+sortname+" "+sortorder+")a_)b_ WHERE b_.row_num <="+top;
		else if(type.indexOf("SQL SERVER")>=0){
			int orderBy=sql.toLowerCase().lastIndexOf("order by");
			String temp="";
			if(orderBy!=-1)
				temp+=sql.substring(orderBy);
			sql="SELECT TOP "+top+" "+sql.substring(7)+" order by "+sortname+" "+sortorder;
		}
		else if(type.indexOf("DB2")>=0)
			sql="SELECT * FROM (SELECT rownumber() as RN, "+sql.substring(7)+" order by "+sortname+" "+sortorder+") WHERE RN <= "+top; 
		else
			throw new Exception("========对不起，此数据库暂不被支持智能提取前几条数据========");
		return sql;
	}
	...
	public static Object[] getInsertsFromBean(String dbType,Object bean,Object id) throws Exception {
		...
		if(id!=null){
		   ...
		   sql.append("ID,");
		   sqlParms.append("SEQ_"+CommonProperties.TABSATRT+ bean.getClass().getSimpleName().toUpperCase()+ ".nextval,");
		}			
	...
	public static Object[] getSelectFromBean(String dbType,Object bean) throws Exception {
		Field[] fields = bean.getClass().getDeclaredFields();
		StringBuffer sql = new StringBuffer("SELECT * FROM "+CommonProperties.TABSATRT
	...
	public static Object[] getDeleteFromBean(Object bean) throws Exception {
		Field[] fields = bean.getClass().getDeclaredFields();
		StringBuffer sql = new StringBuffer("DELETE FROM "+CommonProperties.TABSATRT
	...
	public static Object[] getUpdateFromBean(Object bean) throws Exception {
		Field[] fields = bean.getClass().getDeclaredFields();
		StringBuffer sql = new StringBuffer("UPDATE "+CommonProperties.TABSATRT
	...
	public static Object[] getUpdateByClass(Class beanClass,HashMap<String, Object> parms,Object id) throws Exception {
		...
		StringBuffer sql = new StringBuffer("UPDATE "+CommonProperties.TABSATRT+ beanClass.getSimpleName().toUpperCase() + " SET ");
