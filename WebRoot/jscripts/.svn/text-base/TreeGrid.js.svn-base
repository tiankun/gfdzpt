...
TreeGrid = function(_config){
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			var isOpen = __root.find("#" + trid)[0].open;
			isOpen = (isOpen == "Y") ? "N" : "Y";
			__root.find("#" + trid)[0].open = isOpen;
	...
	showSubs = function(_trid){
		var isOpen = __root.find("#" + _trid)[0].open;
		if(isOpen == "Y"){
